...
def initMisc(self):
    ...
    q_list = ["Can protagonist appearance be customized?","Can protagonist name be customized?",
    ...
    helpts = [[None,["If enabled, player has customization options at the start of game", "This includes hair color, skin color, and hair style by default."],["font-size:24px;",None]],
              [None,["If enabled, player can type in their own name","You can edit those settings in the Unit Editor, when editing the protagonist"],["font-size:24px;",None]],
...
def initR(self):
    ...
    q_list = ["When can S level supports (marriage) for the protagonist occur?",
...
def initM(self):
    ...
    q_list = [
              ...
              "Are map battles random or on every level?"
              ]
