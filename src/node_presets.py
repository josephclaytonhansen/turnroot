...
class split_true_false(QWidget):
    def __init__(self, scene, hexe="737466"):
        QObject.__init__(self)
        self.scene = scene
        self.hexe = hexe
        self.title="Split True/False to Events"
        self.inputs = [S_BOOLEAN]
        self.outputs=[S_TRIGGER, S_TRIGGER]
        ...
        label2 = QLabel("If True")
        label2.setAlignment(Qt.AlignRight)
        label3 = QLabel("If False")
        ...
        self.n = Node(self.scene, self.title, self.inputs, self.outputs, self.contents, self.socket_content_index, 230)
        self.n.node_preset = self
    
    def updateEmission(self):
        try:
            if self.n.inputs[0].reception == True:
                self.n.outputs[0].emission = "736574"
                self.n.outputs[1].emission = "736566"
        except:
            pass
...
NODES = {"Math": number_number_math, "Compare Numbers": compare_numbers, "Combat Start": combat_start,
         ...
         "Split True/False to Events": split_true_false}
